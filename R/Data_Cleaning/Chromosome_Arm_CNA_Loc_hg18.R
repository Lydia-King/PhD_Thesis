...
CNA <-
    read.delim(
        "../../data/METABRIC_2019/data_CNA.txt",
        sep = "\t",
        na.strings = c("", " ", "NA")
    )
...
CNA_Loc <- CNA %>% dplyr::select(1:2)
CNA_Loc <- merge(CNA_Loc, ens.gene.ann.hg18, by.x="Hugo_Symbol", by.y = "Gene") %>% 
    distinct(Hugo_Symbol, .keep_all = T)

### What genes are missing
Missing_Genes_1 <-
    CNA$Hugo_Symbol[which(CNA$Hugo_Symbol %!in% ens.gene.ann.hg18$Gene)]
